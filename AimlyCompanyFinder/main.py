...
company_addition_active > 0, then runs the AI discovery pipeline via
the OpenClaw container to find and insert companies into the DB.
...
Environment variables (from root .env via docker-compose env_file):
  ...
  OPENCLAW_URL          — e.g. http://company-finder-openclaw:8000
  OPENCLAW_SERVER_KEY   — server API key for OpenClaw
"""
...
def load_env():
    ...
    if env_path.exists():
        ...
        print(f"Warning: Could not find .env at {env_path} — using system env vars")

load_env()

from core.database.connection import get_connection
from services.company_service import find_companies_by_query
from sub_agents.company_discovery.agent import OpenClawClient

POLL_INTERVAL = 30
BATCH_SIZE    = 10

OPENCLAW_URL        = os.getenv("OPENCLAW_URL",        "http://company-finder-openclaw:8000")
OPENCLAW_SERVER_KEY = os.getenv("OPENCLAW_SERVER_KEY", "")
...
class CompanyFinderWorker:
    ...
    def start(self):
        ...
        self.thread.start()
        print("Company Finder Worker started")

    def stop(self):
        self.running = False
        if self.thread:
            self.thread.join()
        print("Company Finder Worker stopped")
        ...
        for user_job in pending:
            ...
            with self._lock:
                if user_id in self._active_users:
                    print(f"User {user_id} already processing — skipping")
                    continue
    ...
    def _process_user_job(self, user_job: dict):
        ...
        try:
            ...
            try:
                gemini_key = _decrypt_key(metadata["llm_api_key_enc"])
                tavily_key = _decrypt_key(metadata["tavily_api_key_enc"])
            except Exception:
                print(f"User {user_id}: Failed to decrypt API keys — resetting")
                self._reset_user(user_id)
                return

            if not OPENCLAW_SERVER_KEY:
                print(f"User {user_id}: OPENCLAW_SERVER_KEY not set — resetting")
                self._reset_user(user_id)
                return
            ...
            def add_company(company: dict) -> None:
                with get_connection() as conn:
                    ...
                    try:
                        ...
                        conn.rollback()
                        print(f"User {user_id}: Insert failed for {company.get('email')}: {insert_exc}")
...
if __name__ == "__main__":
    ...
    print(f"Worker running — polling every {POLL_INTERVAL} seconds")
