// ============================================================
// OverallStatsCard.tsx - Hero Banner: logo + stats strip
// ============================================================

import React from 'react';
import { useNavigate } from 'react-router-dom';
import {
  HeroCard, HeroTop, HeroLogoBlock, HeroWordmark,
  HeroWordmarkHeavy, HeroWordmarkLight, HeroWordmarkDot,
  HeroTagline, HeroStatsStrip, HeroStatCell,
  HeroStatValue, HeroStatLabel, HeroStatRate,
  NavIconButton, NavIconCount,
  ...
  return (
    <HeroCard theme={theme}>
      ...
      <HeroTop>
        <HeroLogoBlock>
          <HeroWordmark>
            <HeroWordmarkHeavy>Aim</HeroWordmarkHeavy>
            <HeroWordmarkLight theme={theme}>ly</HeroWordmarkLight>
            <HeroWordmarkDot theme={theme}>.</HeroWordmarkDot>
          </HeroWordmark>
          <HeroTagline>Email campaign dashboard</HeroTagline>
        </HeroLogoBlock>

        <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
          {/* History */}
          <NavIconButton theme={theme} onClick={() => navigate('/history')} title="History">
          ...
          {/* Attachments */}
          <NavIconButton theme={theme} onClick={() => navigate('/attachments')} title="Attachments">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
              strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66L9.41 17.41a2 2 0 0 1-2.83-2.83l8.49-8.48"/>
            </svg>
            <NavIconCount>{stats.total_attachments}</NavIconCount>
          </NavIconButton>

          {/* Categories */}
          <NavIconButton theme={theme} onClick={() => navigate('/categories')} title="Categories">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
              strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/>
              <line x1="7" y1="7" x2="7.01" y2="7"/>
            </svg>
            <NavIconCount>{stats.total_categories}</NavIconCount>
          </NavIconButton>

          {/* Companies */}
          <NavIconButton theme={theme} onClick={() => navigate('/companies')} title="Companies">
            <svg width="12" height="15" viewBox="0 0 32 40" fill="none" stroke="currentColor"
            ...
            <NavIconCount>{stats.total_companies}</NavIconCount>
          </NavIconButton>
        </div>
      </HeroTop>
