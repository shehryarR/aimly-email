                          ...
                          <>
                            <div style={{marginBottom:'1rem'}}>
                              <div onDragOver={e=>{e.preventDefault();upd(activeIdx,{isDragOver:true});}} onDragLeave={()=>upd(activeIdx,{isDragOver:false})}
                                ...
                                <input ref={el=>{uploadRefs.current[activeIdx]=el;}} type="file" accept=".pdf,.doc,.docx,.txt,.csv" style={{display:'none'}} onChange={e=>{const f=e.target.files?.[0];if(f)pickFile(activeIdx,f);e.target.value='';}} disabled={entry.uploading}/>
                              </div>
                ...
                <>
                  <div style={{ marginBottom: '1rem' }}>
                    ...
                    >
                      ...
                      <input ref={bulkUpRef} type="file" accept=".pdf,.doc,.docx,.txt,.csv" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) pickBulkFile(f); e.target.value = ''; }} disabled={bulkUploading} />
                    </div>
